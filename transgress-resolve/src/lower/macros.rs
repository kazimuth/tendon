...
pub fn lower_macro_rules(ctx: &ModuleCtx, rules_: &syn::ItemMacro) -> Result<DeclarativeMacroItem, LowerError> {
    ...
    let name = Ident::from(rules_.ident.as_ref().ok_or(LowerError::NotAMacroDeclaration)?);
    let tokens = Tokens::from(&rules_.mac);

    Ok(DeclarativeMacroItem { metadata, name, tokens })
}
